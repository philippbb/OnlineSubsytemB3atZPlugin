...
public:
	...
	EB3atZOnlineDataAdvertisementType::Type AdvertisementType;
	...
	FOnlineSessionSetting() :
		AdvertisementType(EB3atZOnlineDataAdvertisementType::DontAdvertise),
		ID(-1)
	{
	}
	...
	/** Constructor for settings created/defined on the host for a session */
	template<typename Type>
	FOnlineSessionSetting(const Type& InData, EB3atZOnlineDataAdvertisementType::Type InAdvertisementType) :
		Data(InData),
	...
	/** Constructor for settings created/defined on the host for a session */
	template<typename Type>
	FOnlineSessionSetting(const Type& InData, EB3atZOnlineDataAdvertisementType::Type InAdvertisementType, int32 InID) :
		Data(InData),
		AdvertisementType(InAdvertisementType),
		ID(InID)
	{
	...
	*/
	...
	{
		return FString::Printf(TEXT("%s : %s : %d"), *Data.ToString(), EB3atZOnlineDataAdvertisementType::ToString(AdvertisementType), ID);
	}
};
...
private:
	...
	FOnlineSessionSearchParam() :
	   ComparisonOp(EB3atZOnlineComparisonOp::Equals)
	{
	}
...
public:
	...
	EB3atZOnlineComparisonOp::Type ComparisonOp;
	...
	/** Constructor for setting search parameters in a query */
	template<typename Type> 
	FOnlineSessionSearchParam(const Type& InData, EB3atZOnlineComparisonOp::Type InComparisonOp) :
		Data(InData),
	...
	/** Constructor for setting search parameters in a query */
	template<typename Type>
	FOnlineSessionSearchParam(const Type& InData, EB3atZOnlineComparisonOp::Type InComparisonOp, int32 InID) :
		Data(InData),
		ComparisonOp(InComparisonOp),
		ID(InID)
	{
	...
	FString ToString() const
	{
		return FString::Printf(TEXT("Value=%s : %s : %d"), *Data.ToString(), EB3atZOnlineComparisonOp::ToString(ComparisonOp), ID);
	}
};
 ...
 */
class ONLINESUBSYSTEMB3ATZ_API FOnlineSearchSettings
{
public:
	...
	void Set(FName Key, const ValueType& Value, EB3atZOnlineComparisonOp::Type InType, int32 ID);
	 ...
	 */
	template<typename ValueType> 
	void Set(FName Key, const ValueType& Value, EB3atZOnlineComparisonOp::Type InType);
	 ...
	 */
	EB3atZOnlineComparisonOp::Type GetComparisonOp(FName Key) const;
};
...
public:
	...
	/**IP which the host uses for session*/
	int32 CreateSessionIP;
	...
	FOnlineSessionSettingsBeatZ() :
		...
		CreateSessionIP(0)
	{
// 		Set(SETTING_MAPNAME, FString(TEXT("")), EB3atZOnlineDataAdvertisementType::ViaOnlineService);
// 		Set(SETTING_NUMBOTS, 0, EB3atZOnlineDataAdvertisementType::ViaOnlineService);
// 		Set(SETTING_GAMEMODE, FString(TEXT("")), EB3atZOnlineDataAdvertisementType::ViaOnlineService);
	 ...
	 */
	template<typename ValueType>
	void Set(FName Key, const ValueType& Value, EB3atZOnlineDataAdvertisementType::Type InType, int32 InID);
	 ...
	 */
	template<typename ValueType> 
	void Set(FName Key, const ValueType& Value, EB3atZOnlineDataAdvertisementType::Type InType);
	 ...
	 */
	EB3atZOnlineDataAdvertisementType::Type GetAdvertisementType(FName Key) const;
...
protected:
	FNamedOnlineSession() :
		...
		SessionState(EB3atZOnlineSessionState::NoSession)
	{
	}
...
public:
	...
	bool GetJoinability(bool& bPublicJoinable, bool& bFriendJoinable, bool& bInviteOnly, bool& bAllowInvites) const
	{
		...
		if (SessionState != EB3atZOnlineSessionState::NoSession && SessionState != EB3atZOnlineSessionState::Creating && SessionState != EB3atZOnlineSessionState::Destroying)
		{
			bool bAllowJIP = SessionSettings.bAllowJoinInProgress || (SessionState != EB3atZOnlineSessionState::Starting && SessionState != EB3atZOnlineSessionState::InProgress);
			if (bAllowJIP)
			{
...
public:
	...
	/** State of the search */
	EB3atZOnlineAsyncTaskState::Type SearchState;
	...
	/** IP From Host Session received from Master Sever */
	int32 HostSessionAddr;
	/** Port From Host Session received from Master Sever */
	int32 HostSessionPort;

	/** Constructor */
	FOnlineSessionSearchB3atZ() :
		SearchState(EB3atZOnlineAsyncTaskState::NotStarted),
		MaxSearchResults(1),
		bIsLanQuery(false),
		PingBucketSize(0),
		PlatformHash(0),
		...
		HostSessionAddr(0)
	{
		QuerySettings.Set(SETTING_MAPNAME, FString(TEXT("")), EB3atZOnlineComparisonOp::Equals);
		QuerySettings.Set(SEARCH_DEDICATED_ONLY, false, EB3atZOnlineComparisonOp::Equals);
		QuerySettings.Set(SEARCH_EMPTY_SERVERS_ONLY, false, EB3atZOnlineComparisonOp::Equals);
		QuerySettings.Set(SEARCH_SECURE_SERVERS_ONLY, false, EB3atZOnlineComparisonOp::Equals);
	}

	virtual ~FOnlineSessionSearchB3atZ() {}
 ...
 */
void ONLINESUBSYSTEMB3ATZ_API DumpNamedSession(const FNamedOnlineSession* NamedSession);
 ...
 */
void ONLINESUBSYSTEMB3ATZ_API DumpSession(const FOnlineSession* Session);
