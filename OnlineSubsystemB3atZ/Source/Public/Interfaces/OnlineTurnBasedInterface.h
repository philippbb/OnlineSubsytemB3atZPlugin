...
*/
UENUM(BlueprintType)
namespace EB3atZMPMatchOutcome
{
    enum Outcome
    {
        None,
...
public:
    ...
    virtual EB3atZMPMatchOutcome::Outcome GetMatchOutcomeForPlayer(int32 PlayerIndex) const { return EB3atZMPMatchOutcome::None; }
    ...
    virtual void QuitMatch(EB3atZMPMatchOutcome::Outcome Outcome, int32 TurnTimeoutInSeconds, FQuitMatchSignature QuitMatchCallback) {}

	/* Ends the match while setting the match outcome (win/loss/tie) for all players */
	virtual void EndMatch(FEndMatchSignature QuitMatchCallback, EB3atZMPMatchOutcome::Outcome LocalPlayerOutcome, EB3atZMPMatchOutcome::Outcome OtherPlayersOutcome) {}
};
...
public:
	...
	// Accessors referring to an object that inherits from UB3atZTurnBasedMatchInterface that acts like an FTurnBasedEventDelegate
