 ...
 */
DECLARE_MULTICAST_DELEGATE_TwoParams(FOnReadSharedFileComplete, bool, const FB3atZSharedContentHandle&);
 ...
 */
DECLARE_MULTICAST_DELEGATE_FourParams(FOnWriteSharedFileComplete, bool, const FUniqueNetId&, const FString&, const TSharedRef<FB3atZSharedContentHandle>&);
...
public:
	 ...
	 */
	virtual bool GetSharedFileContents(const FB3atZSharedContentHandle& SharedHandle, TArray<uint8>& FileContents) = 0;
	 ...
	 */
	virtual bool ClearSharedFile(const FB3atZSharedContentHandle& SharedHandle) = 0;
	 ...
	 */
	virtual bool ReadSharedFile(const FB3atZSharedContentHandle& SharedHandle) = 0;
	 ...
	 * @param SharedHandle the handle of the read content
	 */
	DEFINE_ONLINE_DELEGATE_TWO_PARAM(OnReadSharedFileComplete, bool, const FB3atZSharedContentHandle&);
	 ...
	 * @param SharedHandle the handle to the shared file, may be platform dependent
	 */
	DEFINE_ONLINE_DELEGATE_FOUR_PARAM(OnWriteSharedFileComplete, bool, const FUniqueNetId&, const FString&, const TSharedRef<FB3atZSharedContentHandle>&);

	/** Interface to get some test content handles */
	virtual void GetDummySharedHandlesForTest(TArray< TSharedRef<FB3atZSharedContentHandle> > & OutHandles) = 0;
};
