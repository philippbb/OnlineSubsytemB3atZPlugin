...
ONLINESUBSYSTEMB3ATZ_API DECLARE_LOG_CATEGORY_EXTERN(LogB3atZOnlineChat, Display, All);

/** Online subsystem stats */
DECLARE_STATS_GROUP(TEXT("Online"), STATGROUP_B3atZOnline, STATCAT_Advanced);
/** Total async thread time */
DECLARE_CYCLE_STAT_EXTERN(TEXT("OnlineAsync"), STAT_B3atZOnline_Async, STATGROUP_B3atZOnline, ONLINESUBSYSTEMB3ATZ_API);
/** Number of async tasks in queue */
DECLARE_DWORD_ACCUMULATOR_STAT_EXTERN(TEXT("NumTasks"), STAT_B3atZOnline_AsyncTasks, STATGROUP_B3atZOnline, ONLINESUBSYSTEMB3ATZ_API);
/** Total time to process session interface */
DECLARE_CYCLE_STAT_EXTERN(TEXT("SessionInt"), STAT_B3atZSession_Interface, STATGROUP_B3atZOnline, ONLINESUBSYSTEMB3ATZ_API);
/** Total time to process both local/remote voice */
DECLARE_CYCLE_STAT_EXTERN(TEXT("VoiceInt"), STAT_B3atZVoice_Interface, STATGROUP_B3atZOnline, ONLINESUBSYSTEMB3ATZ_API);

#define ONLINE_LOG_PREFIX TEXT("OSS: ")
#define UE_LOG_ONLINEB3ATZ(Verbosity, Format, ...) \
{ \
	UE_LOG(LogB3atZOnline, Verbosity, TEXT("%s%s"), ONLINE_LOG_PREFIX, *FString::Printf(Format, ##__VA_ARGS__)); \
}
...
typedef TSharedPtr<class IB3atZOnlineChat, ESPMode::ThreadSafe> IB3atZOnlineChatPtr;
 ...
 */
DECLARE_MULTICAST_DELEGATE_TwoParams(FOnB3atZConnectionStatusChanged, EB3atZOnlineServerConnectionStatus::Type /*LastConnectionState*/, EB3atZOnlineServerConnectionStatus::Type /*ConnectionState*/);
typedef FOnB3atZConnectionStatusChanged::FDelegate FOnB3atZConnectionStatusChangedDelegate;
 ...
 */
DECLARE_MULTICAST_DELEGATE_TwoParams(FOnB3atZOnlineEnvironmentChanged, EOnlineEnvironmentB3atZ::Type /*LastEnvironment*/, EOnlineEnvironmentB3atZ::Type /*Environment*/);
typedef FOnB3atZOnlineEnvironmentChanged::FDelegate FOnB3atZOnlineEnvironmentChangedDelegate;
...
/**
 *	OnlineSubsystemB3atZ - Series of interfaces to support communicating with various web/platform layer services
 */
class ONLINESUBSYSTEMB3ATZ_API IOnlineSubsystemB3atZ
{
protected:
	/** Hidden on purpose */
...
public:
	 ...
	 */
	static IOnlineSubsystemB3atZ* Get(const FName& SubsystemName = NAME_None)
	{
		UE_LOG(LogB3atZOnline, VeryVerbose, TEXT("OSB.h Get Incoming Name is %s"), *SubsystemName.ToString());

		static const FName OnlineSubsystemModuleName = TEXT("OnlineSubsystemB3atZ");
		FOnlineSubsystemB3atZModule& OSSModule = FModuleManager::GetModuleChecked<FOnlineSubsystemB3atZModule>(OnlineSubsystemModuleName);
		
		//FFrame::KismetExecutionMessage(*FString::Printf(TEXT("OnlineSubsystemB3atZ.h module check valid?"), FunctionContext), ELogVerbosity::VeryVerbose);
	 ...
	 * @return pointer to the appropriate online subsystem
	 */
	static IOnlineSubsystemB3atZ* GetByPlatform(bool bAutoLoad=true)
	{
		if (PLATFORM_PS4)
		{
			if (bAutoLoad || IOnlineSubsystemB3atZ::IsLoaded(PS4_SUBSYSTEM))
			{
				return IOnlineSubsystemB3atZ::Get(PS4_SUBSYSTEM);
			}
		}
		else if (PLATFORM_XBOXONE)
		{
			if (bAutoLoad || IOnlineSubsystemB3atZ::IsLoaded(LIVE_SUBSYSTEM))
			{
				return IOnlineSubsystemB3atZ::Get(LIVE_SUBSYSTEM);
			}
		}
		else if (PLATFORM_ANDROID)
		{
			if (bAutoLoad || IOnlineSubsystemB3atZ::IsLoaded(GOOGLEPLAY_SUBSYSTEM))
			{
				return IOnlineSubsystemB3atZ::Get(GOOGLEPLAY_SUBSYSTEM);
			}
		}
		else if (PLATFORM_IOS)
		{
			if (bAutoLoad || IOnlineSubsystemB3atZ::IsLoaded(IOS_SUBSYSTEM))
			{
				return IOnlineSubsystemB3atZ::Get(IOS_SUBSYSTEM);
			}
		}
		return nullptr;
  	}
	...
	{
		if (FModuleManager::Get().IsModuleLoaded("OnlineSubsystemB3atZ"))
		{
			FOnlineSubsystemB3atZModule& OSSModule = FModuleManager::GetModuleChecked<FOnlineSubsystemB3atZModule>("OnlineSubsystemB3atZ");
	...
	*/
	 ...
	 * @return Interface pointer for the appropriate online user service
	 */
	virtual IB3atZOnlineChatPtr GetChatInterface() const = 0;
	...
	*/
	 ...
	 * @param ConnectionState current state of the connection
	 */
	DEFINE_ONLINE_DELEGATE_TWO_PARAM(OnB3atZConnectionStatusChanged, EB3atZOnlineServerConnectionStatus::Type /*LastConnectionState*/, EB3atZOnlineServerConnectionStatus::Type /*ConnectionState*/);

	/**
	 * @return the current environment being used for the online platform
	 */
	virtual EOnlineEnvironmentB3atZ::Type GetOnlineEnvironment() const = 0;
	 ...
	 * @param Environment - current online environment
	 */
	DEFINE_ONLINE_DELEGATE_TWO_PARAM(OnB3atZOnlineEnvironmentChanged, EOnlineEnvironmentB3atZ::Type /*LastEnvironment*/, EOnlineEnvironmentB3atZ::Type /*Environment*/);
...
typedef TSharedPtr<IOnlineSubsystemB3atZ, ESPMode::ThreadSafe> IOnlineSubsystemB3atZPtr;

/**
 * Interface for creating the actual online subsystem instance for a given platform
 * all modules must implement this
 */
class ONLINESUBSYSTEMB3ATZ_API IB3atZOnlineFactory
{
public:

	IB3atZOnlineFactory() {}
	virtual ~IB3atZOnlineFactory() {}
	 ...
	 */
	virtual IOnlineSubsystemB3atZPtr CreateSubsystem(FName InstanceName) = 0;
};
 ...
 */
ONLINESUBSYSTEMB3ATZ_API int32 GetBuildUniqueId();
 ...
 */
ONLINESUBSYSTEMB3ATZ_API bool IsPlayerInSessionImpl(class IOnlineSession* SessionInt, FName SessionName, const FUniqueNetId& UniqueId);
