...
#ifdef ONLINESUBSYSTEMB3ATZUTILS_API

/** @return an initialized audio component specifically for use with VoIP */
ONLINESUBSYSTEMB3ATZUTILS_API class UAudioComponent* CreateVoiceAudioComponent(uint32 SampleRate);

/** @return the world associated with a named online subsystem instance */
ONLINESUBSYSTEMB3ATZUTILS_API UWorld* GetWorldForOnline(FName InstanceName);
 ...
 */
ONLINESUBSYSTEMB3ATZUTILS_API int32 GetPortFromNetDriver(FName InstanceName);
...
protected:
	/** Hidden on purpose */
	IOnlineSubsystemB3atZUtils() {}

public:

	virtual ~IOnlineSubsystemB3atZUtils() {}
...
{ \
	IOnlineSubsystemB3atZ* OSS = Online::GetSubsystem(World, SubsystemName); \
...
namespace Online
{
	 ...
	 */
	static IOnlineSubsystemB3atZ* GetSubsystem(UWorld* World, const FName& SubsystemName = NAME_None)
	{
		UE_LOG(LogB3atZOnline, VeryVerbose, TEXT("OSBU.h GetSubsystem with name %s"), *SubsystemName.ToString());
		...
		return IOnlineSubsystemB3atZ::Get(Identifier); 
#else
		return IOnlineSubsystemB3atZ::Get(SubsystemName); 
#endif
	}
	...
	{
		UE_LOG(LogB3atZOnline, VeryVerbose, TEXT("OSBU.h is loaded"));
#if UE_EDITOR // at present, multiple worlds are only possible in the editor
		FName Identifier = SubsystemName;
		if (World != NULL)
		{
		...
		return IOnlineSubsystemB3atZ::IsLoaded(SubsystemName);
#else
		return IOnlineSubsystemB3atZ::IsLoaded(SubsystemName);
#endif
	}
	...
	static IOnlineSubsystemB3atZUtils* GetUtils()
	{
		UE_LOG(LogB3atZOnline, VeryVerbose, TEXT("OSBU.h GetUtils"));
		static const FName OnlineSubsystemModuleName = TEXT("OnlineSubsystemB3atZUtils");
		FOnlineSubsystemB3atZUtilsModule* OSSUtilsModule = FModuleManager::GetModulePtr<FOnlineSubsystemB3atZUtilsModule>(OnlineSubsystemModuleName);
