...
#include "../OnlineSubsystemB3atZUtils/Public/B3atZOnlineBeaconClient.h"

#include "B3atZPartyBeaconClient.generated.h"
 ...
 */
UENUM()
enum class EDirectClientRequestType : uint8
{
	/** None pending */
	NonePending,
...
inline const TCHAR* ToString(EDirectClientRequestType RequestType)
{
	switch (RequestType)
	{
	case EDirectClientRequestType::NonePending:
	{
		return TEXT("No Request Pending");
	}
	case EDirectClientRequestType::ExistingSessionReservation:
	{
		return TEXT("Existing Session Reservation");
	}
	case EDirectClientRequestType::ReservationUpdate:
	{
		return TEXT("Reservation Update");
	}
	case EDirectClientRequestType::EmptyServerReservation:
	{
		return TEXT("Empty Server Reservation");
	}
	case EDirectClientRequestType::Reconnect:
	{
 ...
 */
DECLARE_DELEGATE_OneParam(FOnReservationRequestComplete, EB3atZPartyReservationResult::Type /** ReservationResponse */);
...
class ONLINESUBSYSTEMB3ATZUTILS_API AB3atZPartyBeaconClient : public AB3atZOnlineBeaconClient
{
	 ...
	 */
	virtual bool RequestReservation(const FString& ConnectInfoStr, const FString& InSessionId, const FUniqueNetIdRepl& RequestingPartyLeader, const TArray<FB3atZPlayerReservation>& PartyMembers);
	 ...
	 * @return true if the request able to be sent, false if it failed to send
	 */
	virtual bool RequestReservation(const FOnlineSessionSearchResult& DesiredHost, const FUniqueNetIdRepl& RequestingPartyLeader, const TArray<FB3atZPlayerReservation>& PartyMembers);
	 ...
	 * @return true if the request able to be sent, false if it failed to send
	 */
	virtual bool RequestReservationUpdate(const FUniqueNetIdRepl& RequestingPartyLeader, const TArray<FB3atZPlayerReservation>& PlayersToAdd);
	 ...
	 * @return true if the request able to be sent, false if it failed to send
	 */
	virtual bool RequestReservationUpdate(const FString& ConnectInfoStr, const FString& InSessionId, const FUniqueNetIdRepl& RequestingPartyLeader, const TArray<FB3atZPlayerReservation>& PlayersToAdd);
	 ...
	 * @return true if the request able to be sent, false if it failed to send
	 */
	virtual bool RequestReservationUpdate(const FOnlineSessionSearchResult& DesiredHost, const FUniqueNetIdRepl& RequestingPartyLeader, const TArray<FB3atZPlayerReservation>& PlayersToAdd);
	...
	virtual void ClientReservationResponse(EB3atZPartyReservationResult::Type ReservationResponse);
	 ...
	 */
	UFUNCTION(client, reliable)
	virtual void ClientCancelReservationResponse(EB3atZPartyReservationResult::Type ReservationResponse);
	...
	*/
	const FB3atZPartyReservation& GetPendingReservation() const { return PendingReservation; }
...
protected:
	...
	UPROPERTY()
	FB3atZPartyReservation PendingReservation;

	/** Type of request currently being handled by this client beacon */
	UPROPERTY()
	EDirectClientRequestType RequestType;
	...
	void ProcessReservationResponse(EB3atZPartyReservationResult::Type ReservationResponse);
	/** Process a response to our CancelReservation request to the server */
	void ProcessCancelReservationResponse(EB3atZPartyReservationResult::Type ReservationResponse);
	...
	virtual void ServerReservationRequest(const FString& SessionId, const FB3atZPartyReservation& Reservation);
	 ...
	 */
	UFUNCTION(server, reliable, WithValidation)
	virtual void ServerUpdateReservationRequest(const FString& SessionId, const FB3atZPartyReservation& ReservationUpdate);
