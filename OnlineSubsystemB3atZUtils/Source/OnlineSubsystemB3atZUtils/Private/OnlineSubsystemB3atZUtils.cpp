...
UAudioComponent* CreateVoiceAudioComponent(uint32 SampleRate)
{
	...
	if (GEngine != nullptr)
	{
		if (FAudioDevice* AudioDevice = GEngine->GetMainAudioDevice())
		{
			...
			{
				UE_LOG(LogVoiceDecode, VeryVerbose, TEXT("Unable to create voice audio component!"));
			}
		}
	}
...
UWorld* GetWorldForOnline(FName InstanceName)
{
	UWorld* World = NULL;
#ifdef WITH_EDITOR
	if (InstanceName != FOnlineSubsystemB3atZImpl::DefaultInstanceName && InstanceName != NAME_None)
	{
...
{
	UE_LOG(LogNet, VeryVerbose, TEXT("OSBU GetPortFromNetDriver"));
	int32 Port = 0;
#if WITH_ENGINE
	if (GEngine)
	{
		UE_LOG(LogNet, VeryVerbose, TEXT("OSBU GetPortFromNetDriver GEngine valid"));
		...
		if (NetDriver)
		{
			UE_LOG(LogNet, VeryVerbose, TEXT("OSBU GetPortFromNetDriver NamedNetDriverFound"));
		}

		if (NetDriver && NetDriver->GetNetMode() < NM_Client)
		{
			UE_LOG(LogNet, VeryVerbose, TEXT("OSBU GetPortFromNetDriver NamedNetDriverFound and this is not client"));
...
{
	UE_LOG(LogB3atZOnline, VeryVerbose, TEXT("OSBU HandleSessionCommands"));
...
bool HandleVoiceCommands(UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar)
{
	...
	if (FParse::Command(&Cmd, TEXT("DUMP")))
	{
		...
		{
			UE_LOG(LogVoice, VeryVerbose, TEXT("Missing bRequiresPushToTalk key in [/Script/Engine.GameSession] of DefaultGame.ini"));
		}

		int32 MaxLocalTalkers = 0;
		if (!GConfig->GetInt(TEXT("OnlineSubsystemB3atZ"), TEXT("MaxLocalTalkers"), MaxLocalTalkers, GEngineIni))
		{
			UE_LOG(LogVoice, VeryVerbose, TEXT("Missing MaxLocalTalkers key in OnlineSubsystem of DefaultEngine.ini"));
		}

		int32 MaxRemoteTalkers = 0;
		if (!GConfig->GetInt(TEXT("OnlineSubsystemB3atZ"), TEXT("MaxRemoteTalkers"), MaxRemoteTalkers, GEngineIni))
		{
			UE_LOG(LogVoice, VeryVerbose, TEXT("Missing MaxRemoteTalkers key in OnlineSubsystem of DefaultEngine.ini"));
		}
		
		float VoiceNotificationDelta = 0.0f;
		if (!GConfig->GetFloat(TEXT("OnlineSubsystemB3atZ"), TEXT("VoiceNotificationDelta"), VoiceNotificationDelta, GEngineIni))
		{
			UE_LOG(LogVoice, VeryVerbose, TEXT("Missing VoiceNotificationDelta key in OnlineSubsystem of DefaultEngine.ini"));
		}

		bool bHasVoiceInterfaceEnabled = false;
		if (!GConfig->GetBool(TEXT("OnlineSubsystemB3atZ"), TEXT("bHasVoiceEnabled"), bHasVoiceInterfaceEnabled, GEngineIni))
		{
			UE_LOG(LogVoice, Log, TEXT("Voice interface disabled by config [OnlineSubsystem].bHasVoiceEnabled"));
		}

		bool bDuckingOptOut = false;
		if (!GConfig->GetBool(TEXT("OnlineSubsystemB3atZ"), TEXT("bDuckingOptOut"), bDuckingOptOut, GEngineIni))
		{
...
static bool OnlineExec( UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar )
{
	...
	if (FParse::Command(&Cmd, TEXT("ONLINE")))
	{
		...
		IOnlineSubsystemB3atZ* OnlineSub = NULL;
		// If the exec requested a specific subsystem, the grab that one for routing
		if (SubName.Len())
		{
			UE_LOG(LogB3atZOnline, VeryVerbose, TEXT("OSBU before GetSubsystem subname longer than 0"));
			OnlineSub = Online::GetSubsystem(InWorld, *SubName);
		}
		// Otherwise use the default subsystem and route to that
		else
		{
			UE_LOG(LogB3atZOnline, VeryVerbose, TEXT("OSBU before GetSubsystem subname not longer than 0"));
...
{
	UE_LOG(LogB3atZOnline, VeryVerbose, TEXT("OSBU OSBPCallHelper"));
	if (OnlineSub == nullptr)
	{
		FFrame::KismetExecutionMessage(*FString::Printf(TEXT("%s - Invalid or uninitialized OnlineSubsystem"), FunctionContext), ELogVerbosity::VeryVerbose);
	}
}
...
void FOnlineSubsystemBPCallHelper::QueryIDFromPlayerController(APlayerController* PlayerController)
{
	...
	if (APlayerState* PlayerState = (PlayerController != NULL) ? PlayerController->PlayerState : NULL)
	{
		...
		if (!UserID.IsValid())
		{
			FFrame::KismetExecutionMessage(*FString::Printf(TEXT("%s - Cannot map local player to unique net ID"), FunctionContext), ELogVerbosity::VeryVerbose);
		}
	}
	else
	{
		FFrame::KismetExecutionMessage(*FString::Printf(TEXT("%s - Invalid player state"), FunctionContext), ELogVerbosity::VeryVerbose);
	}
}
