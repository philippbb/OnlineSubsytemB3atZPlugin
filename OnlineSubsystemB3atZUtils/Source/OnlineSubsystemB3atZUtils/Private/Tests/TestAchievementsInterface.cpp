...
{
	UE_LOG(LogB3atZOnline, Display, TEXT("FTestAchievementsInterface::Test"));

	IOnlineSubsystemB3atZ* OnlineSub = Online::GetSubsystem(InWorld, FName(*SubsystemName));
	check(OnlineSub); 
	...
	{
		UE_LOG(LogB3atZOnline, Warning, TEXT("TEST FAILED: OSS [%s] does not have a valid achievement interface or identity interface for this test to run."), *SubsystemName );
		delete this;
	}
}
...
{
	UE_LOG(LogB3atZOnline, Display, TEXT("FTestAchievementsInterface::OnQueryAchievementsComplete"));

	if (bWasSuccessful)
	{
		UE_LOG(LogB3atZOnline, Display, TEXT("Loaded Achievements"));

		TArray<FOnlineAchievement> PlayerAchievements;
		if (OnlineAchievements->GetCachedAchievements(*UserId.Get(), PlayerAchievements) != EB3atZOnlineCachedResult::Success || PlayerAchievements.Num() == 0)
		{
			UE_LOG(LogB3atZOnline, Warning, TEXT("TEST FAILED: Either GetCachedAchievements() failed or number of achievements is 0"));
			delete this;
			return;
		}

		UE_LOG(LogB3atZOnline, Display, TEXT("Number of Achievements: %d"), PlayerAchievements.Num());
		for(int32 Idx = 0; Idx < PlayerAchievements.Num(); ++Idx)
		{
			UE_LOG(LogB3atZOnline, Display, TEXT(" Achievement %d: %s"), Idx, *PlayerAchievements[ Idx ].ToDebugString());
		}
	...
	else
	{
		...
		UE_LOG(LogB3atZOnline, Display, TEXT("TEST FAILED: Failed to Load Achievements"));
		delete this;
	}
}
...
{
	UE_LOG(LogB3atZOnline, Display, TEXT("FTestAchievementsInterface::OnQueryAchievementDescriptionsComplete"));

	if (bWasSuccessful)
	{
		UE_LOG(LogB3atZOnline, Display, TEXT("Loaded Achievement descriptions"));
		...
		for(int32 Idx = 0; Idx < PlayerAchievements.Num(); ++Idx)
		{
			...
			if (OnlineAchievements->GetCachedAchievementDescription(PlayerAchievements[ Idx ].Id, Desc) != EB3atZOnlineCachedResult::Success)
			{
				UE_LOG(LogB3atZOnline, Warning, TEXT("Failed to GetCachedAchievementDescription() for achievement '%s'"),
					*PlayerAchievements[ Idx ].Id
					);
				delete this;
				return;
			}
			UE_LOG(LogB3atZOnline, Display, TEXT(" Descriptor for achievement '%s': %s"), *PlayerAchievements[ Idx ].Id, *Desc.ToDebugString());
		}
	...
	{
		// if our test failed, be sure to clean itself up
		UE_LOG(LogB3atZOnline, Warning, TEXT("TEST FAILED: Failed to Load Achievement descriptions"));
		delete this;
	}
}
...
{
	UE_LOG(LogB3atZOnline, Display, TEXT("FTestAchievementsInterface::OnAchievementsWritten( bWasSuccessful = %s )"), bWasSuccessful ? TEXT("true") : TEXT("false"));
	...
	if (bWasSuccessful && WriteObject->WriteState == EB3atZOnlineAsyncTaskState::Done)
	{
#if !UE_BUILD_SHIPPING
		OnlineAchievements->ResetAchievements(*UserId.Get());
#endif // !UE_BUILD_SHIPPING
		// TODO: spin for a while?
		UE_LOG(LogB3atZOnline, Display, TEXT("TEST COMPLETED SUCCESSFULLY."));
	}
	else
	{
		if (WriteObject->WriteState != EB3atZOnlineAsyncTaskState::Done)
		{
			UE_LOG(LogB3atZOnline, Warning, TEXT("TEST FAILED: WriteObject->WriteState is not in state %s, but instead %s"),
				EB3atZOnlineAsyncTaskState::ToString(EB3atZOnlineAsyncTaskState::Done),
				EB3atZOnlineAsyncTaskState::ToString(WriteObject->WriteState)
				);
		}

		if (!bWasSuccessful)
		{
			UE_LOG(LogB3atZOnline, Warning, TEXT("TEST FAILED: Write did not complete successfully") );
		}
	}
...
{
	UE_LOG(LogB3atZOnline, Display, TEXT("Achievement Unlocked - %s"), *AchievementId);
}
