 ...
 */
 class FTestCloudInterface : public FTickerObjectBase
 {
	...
	TArray< TSharedRef<FB3atZSharedContentHandle> > CloudFileHandles;
	/** Static array of shared handles from another user */
	TArray< TSharedRef<FB3atZSharedContentHandle> > RandomSharedFileHandles;
	 ...
	 */
	void OnWriteSharedCloudFileComplete(bool bWasSuccessful, const FUniqueNetId& UserId, const FString& FileName, const TSharedRef<FB3atZSharedContentHandle>& SharedHandle);
	 ...
	 */
	void OnReadEnumeratedSharedFileCompleteDelegate(bool bWasSuccessful, const FB3atZSharedContentHandle& SharedHandle);
