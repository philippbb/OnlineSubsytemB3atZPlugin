...
void FTestCloudInterface::Test(UWorld* InWorld)
{
	...
	if (UserId.IsValid())
	{
		...
		{
			UE_LOG(LogB3atZOnline, Warning, TEXT("Cloud test failed.  Cloud API not supported."));
			delete this;
		}
	}
	else
	{
		UE_LOG(LogB3atZOnline, Warning, TEXT("Cloud test failed.  No logged in user."));
		delete this;
	}
}
...
bool FTestCloudInterface::Tick( float DeltaTime )
{
	if (TestPhase != LastTestPhase)
	{
		...
		if (!SharedCloud.IsValid() && 
			TestPhase > 4)
		{
			UE_LOG(LogB3atZOnline, Log, TEXT("Skipping shared cloud tests"));
			TestPhase = 10;
		}
...
bool FTestCloudInterface::Cleanup()
{
	...
	{
		UE_LOG(LogB3atZOnline, Log, TEXT("Failed to cleanup user files"));
		bSuccess = false;
	}
	if (SharedCloud.IsValid() &&
		!SharedCloud->ClearSharedFiles())
	{
		UE_LOG(LogB3atZOnline, Log, TEXT("Failed to cleanup shared files"));
		bSuccess = false;
	}
...
{
	UE_LOG(LogB3atZOnline, Log, TEXT("------------------------------------------------"));
	UE_LOG(LogB3atZOnline, Log, TEXT("OnEnumerateUserFilesComplete Success:%d UserId:%s"), bWasSuccessful, *InUserId.ToDebugString());
	bOverallSuccess = bOverallSuccess && bWasSuccessful;

	TArray<FB3atZCloudFileHeader> UserFiles;
	...
	{
		UE_LOG(LogB3atZOnline, Log, TEXT("\t%d FileName:%s DLName:%s Hash:%s Size:%d"),
...
{
	UE_LOG(LogB3atZOnline, Log, TEXT("------------------------------------------------"));
	UE_LOG(LogB3atZOnline, Log, TEXT("Writing %d files to the cloud for user %s"), FileCount, *InUserId.ToDebugString());
...
{
	UE_LOG(LogB3atZOnline, Log, TEXT("------------------------------------------------"));
	UE_LOG(LogB3atZOnline, Log, TEXT("Writing %d files to the cloud and sharing for user %s"), FileCount, *InUserId.ToDebugString());
...
{
	UE_LOG(LogB3atZOnline, Log, TEXT("Write user file complete Success:%d UserId:%s FileName:%s"), bWasSuccessful, *InUserId.ToDebugString(), *FileName);
	...
	{
		UE_LOG(LogB3atZOnline, Log, TEXT("Write %d User Files Complete!"), NumWrittenCount);
		UE_LOG(LogB3atZOnline, Log, TEXT("------------------------------------------------"));
...
FDelegateHandle FTestCloudInterface::ReadEnumeratedUserFiles(FOnReadUserFileCompleteDelegate& Delegate)
{
	...
	UE_LOG(LogB3atZOnline, Log, TEXT("------------------------------------------------"));
	UE_LOG(LogB3atZOnline, Log, TEXT("Reading %d enumerated files for user %s"), ReadUserFileCount, *UserId->ToDebugString());
	...
	if (ReadUserFileCount > 0)
	{
		...
		{
			UE_LOG(LogB3atZOnline, Log, TEXT("\tFileName:%s Size:%d"), *UserFiles[Idx].FileName, UserFiles[Idx].FileSize);
...
void FTestCloudInterface::OnReadEnumeratedUserFilesComplete(bool bWasSuccessful, const FUniqueNetId& InUserId, const FString& FileName)
{
	...
	UE_LOG(LogB3atZOnline, Log, TEXT("Read user file complete Success:%d UserId:%s FileName:%s Size:%d"), bWasSuccessful, *InUserId.ToDebugString(), *FileName, FileSize);
	bOverallSuccess = bOverallSuccess && bWasSuccessful;
	...
	{
		UE_LOG(LogB3atZOnline, Log, TEXT("Read %d User Files Complete!"), NumReadCount);
		UE_LOG(LogB3atZOnline, Log, TEXT("------------------------------------------------"));
...
FDelegateHandle FTestCloudInterface::DeleteEnumeratedUserFiles(bool bCloudDelete, bool bLocalDelete, FOnDeleteUserFileCompleteDelegate& Delegate)
{
	...
	UE_LOG(LogB3atZOnline, Log, TEXT("------------------------------------------------"));
	UE_LOG(LogB3atZOnline, Log, TEXT("Deleting %d files from the cloud for user %s CLOUD: %s LOCAL: %s"), DeleteUserFileCount, *UserId->ToDebugString(), bCloudDelete ? TEXT("true") : TEXT("false"), bLocalDelete ? TEXT("true") : TEXT("false"));
	FDelegateHandle Result;
...
{
	UE_LOG(LogB3atZOnline, Log, TEXT("Delete user file complete Success:%d UserId:%s FileName:%s"), bWasSuccessful, *InUserId.ToDebugString(), *FileName);
	bOverallSuccess = bOverallSuccess && bWasSuccessful;
	...
	{
		UE_LOG(LogB3atZOnline, Log, TEXT("Delete %d User Files Complete!"), NumDeletedCount);
		UE_LOG(LogB3atZOnline, Log, TEXT("------------------------------------------------"));
...
void FTestCloudInterface::OnWriteSharedCloudFileComplete(bool bWasSuccessful, const FUniqueNetId& InUserId, const FString& FileName, const TSharedRef<FB3atZSharedContentHandle>& SharedHandle)
{
	UE_LOG(LogB3atZOnline, Log, TEXT("Write shared file complete Success:%d UserId:%s FileName:%s SharedHandle:%s"), bWasSuccessful, *InUserId.ToDebugString(), *FileName, *SharedHandle->ToDebugString());
	bOverallSuccess = bOverallSuccess && bWasSuccessful;
	...
	{
		UE_LOG(LogB3atZOnline, Log, TEXT("Write %d Shared Files Complete!"), NumWrittenCount);
		UE_LOG(LogB3atZOnline, Log, TEXT("------------------------------------------------"));
...
FDelegateHandle FTestCloudInterface::ReadEnumeratedSharedFiles(bool bUseRandom, FOnReadSharedFileCompleteDelegate& Delegate)
{
	...
	UE_LOG(LogB3atZOnline, Log, TEXT("------------------------------------------------"));
	UE_LOG(LogB3atZOnline, Log, TEXT("Reading %d enumerated shared files"), ReadSharedFileCount);
	...
	if (ReadSharedFileCount > 0)
	{
		...
		for (int32 Idx=0; Idx < ReadSharedFileCount; Idx++)
		{
			if (bUseRandom)
			{
				UE_LOG(LogB3atZOnline, Log, TEXT("\tHandle:%s"), *RandomSharedFileHandles[Idx]->ToDebugString());
				SharedCloud->ReadSharedFile(*RandomSharedFileHandles[Idx]);
			}
			else
			{
				UE_LOG(LogB3atZOnline, Log, TEXT("\tHandle:%s"), *CloudFileHandles[Idx]->ToDebugString());
...
void FTestCloudInterface::OnReadEnumeratedSharedFileCompleteDelegate(bool bWasSuccessful, const FB3atZSharedContentHandle& SharedHandle)
{
	UE_LOG(LogB3atZOnline, Log, TEXT("Read shared file complete Success:%d SharedHandle:%s"), bWasSuccessful, *SharedHandle.ToDebugString());
	bOverallSuccess = bOverallSuccess && bWasSuccessful;
	...
	{
		UE_LOG(LogB3atZOnline, Log, TEXT("Read %d Shared Files Complete!"), NumReadCount);
		UE_LOG(LogB3atZOnline, Log, TEXT("------------------------------------------------"));
