...
void FTestIdentityInterface::OnLoginComplete(int32 LocalUserNum, bool bWasSuccessful, const FUniqueNetId& UserId, const FString& Error)
{
	if (bWasSuccessful)
	{
		UE_LOG(LogB3atZOnline, Display, TEXT("Successful logged in user. UserId=[%s] "), 
	...
	{
		UE_LOG(LogB3atZOnline, Error, TEXT("Failed to log in new user. Error=[%s]"), 
			*Error);
...
void FTestIdentityInterface::OnLogoutComplete(int32 LocalUserNum, bool bWasSuccessful)
{
	if (bWasSuccessful)
	{
		UE_LOG(LogB3atZOnline, Display, TEXT("Successful logged out user. LocalUserNum=[%d] "),
			LocalUserNum);
	}
	else if (!bIsUserLoggedIn)
	{
		UE_LOG(LogB3atZOnline, Display, TEXT("User is not logged in to be able to be logged out."));
	}
	// If the user was logged out at the start of the test then there will be nothing to log out.
	else
	{
		UE_LOG(LogB3atZOnline, Error, TEXT("Failed to log out user."));
	}
...
bool FTestIdentityInterface::IsTheUserLoggedIn()
{
	...
	ELoginStatusB3atZ::Type CurrentLoginStatus = TempInterfacePointer->GetLoginStatus(LocalUserIdx);
	bIsUserLoggedIn = false;

	if ( CurrentLoginStatus == ELoginStatusB3atZ::LoggedIn )
	{
