...
void FTestFriendsInterface::OnReadFriendsComplete(int32 LocalPlayer, bool bWasSuccessful, const FString& ListName, const FString& ErrorStr)
{
	...
	if (bWasSuccessful)
	{
		...
		if (OnlineSub->GetFriendsInterface()->GetFriendsList(LocalPlayer, ListName, Friends))
		{
			...
			{
				const FB3atZOnlineFriend& Friend = *Friends[Index];
				...
				UE_LOG(LogB3atZOnline, Log,
					TEXT("\t\t Invite status (%s)"), EB3atZInviteStatus::ToString(Friend.GetInviteStatus()));
				UE_LOG(LogB3atZOnline, Log,
				...
				if (Friend.GetInviteStatus() == EB3atZInviteStatus::PendingInbound)
				{
...
void FTestFriendsInterface::OnQueryRecentPlayersComplete(const FUniqueNetId& UserId, const FString& Namespace, bool bWasSuccessful, const FString& ErrorStr)
{
	...
	if (bWasSuccessful)
	{
		TArray< TSharedRef<FB3atZOnlineRecentPlayer> > Players;
		// Grab the friends data so we can print it out
...
{
	UE_LOG(LogB3atZOnline, Log, TEXT("AcceptInvite() for player (%d) from friend (%s) was success=%d. %s"), 
...
{
	UE_LOG(LogB3atZOnline, Log, TEXT("SendInvite() for player (%d) to friend (%s) was success=%d. %s"), 
		LocalPlayer, *FriendId.ToDebugString(), bWasSuccessful, *ErrorStr);
...
{
	UE_LOG(LogB3atZOnline, Log, TEXT("DeleteFriend() for player (%d) to friend (%s) was success=%d. %s"), 
		LocalPlayer, *FriendId.ToDebugString(), bWasSuccessful, *ErrorStr);
...
{
	UE_LOG(LogB3atZOnline, Log, TEXT("QueryBlockedPlayers() for player (%s) was success=%d"), *UserId.ToDebugString(), bWasSuccessful);
	bQueryBlockedPlayers = false;

	if (bWasSuccessful)
	{
		TArray<TSharedRef<FB3atZOnlineBlockedPlayer>> BlockedPlayers;
		if (OnlineSub->GetFriendsInterface()->GetBlockedPlayers(UserId, BlockedPlayers))
		{
			UE_LOG(LogB3atZOnline, Log, TEXT("GetBlockedPlayers() (%s) returned %d blocked users"), *UserId.ToDebugString(), BlockedPlayers.Num());

			for (int32 Index = 0; Index < BlockedPlayers.Num(); Index++)
			{
				const FB3atZOnlineBlockedPlayer& BlockedPlayer = *BlockedPlayers[Index];
				UE_LOG(LogB3atZOnline, Log, TEXT("\t%s (%s) is blocked"), *BlockedPlayer.GetRealName(), *BlockedPlayer.GetUserId()->ToDebugString());
			}
		}
		else
		{
			UE_LOG(LogB3atZOnline, Log, TEXT("GetBlockedPlayers() for player %s failed"), *UserId.ToDebugString());
		}
	}
