...
{
	IOnlineSubsystemB3atZ* OSS = Online::GetSubsystem(InWorld, SubsystemName.Len() ? FName(*SubsystemName, FNAME_Find) : NAME_None);
	...
	if (UserId.IsValid() && UserId->IsValid())
	{
		...
		{
			UE_LOG(LogB3atZOnline, Warning, TEXT("Entitlement test failed. Failed to get entitlement service API"));
	...
	{
		UE_LOG(LogB3atZOnline, Warning, TEXT("Entitlement test failed. No logged in user"));
...
{
	UE_LOG(LogB3atZOnline, Log, TEXT("enumerated entitlements. UserId=%s Result=%s Error=[%s]"), 
	...
	for (auto It = Entitlements.CreateConstIterator(); It; ++It)
	{
		...
		UE_LOG(LogB3atZOnline, Log, TEXT("	entitlement id=%s name=%s"),
			*Entry->Id, *Entry->Name);
	}
