...
{
	UE_LOG(LogB3atZOnline, Verbose, TEXT("FTestSharingInterface::FTestSharingInterface"));
...
{
	UE_LOG(LogB3atZOnline, Verbose, TEXT("FTestSharingInterface::Test"));
	...
	TestStatusUpdate.PostPrivacy = EB3atZOnlineStatusUpdatePrivacy::OnlyMe;
	if( bWithImage )
	{
...
{
	UE_LOG(LogB3atZOnline, Verbose, TEXT("FTestSharingInterface::RequestPermissionsToSharePosts"));
	...
	EB3atZOnlineSharingPublishingCategory::Type PublishPermissions = EB3atZOnlineSharingPublishingCategory::Posts;
...
{
	UE_LOG(LogB3atZOnline, Display, TEXT("FTestSharingInterface::OnStatusPostingPermissionsUpdated() - %d"), bWasSuccessful);
...
{
	UE_LOG(LogB3atZOnline, Verbose, TEXT("FTestSharingInterface::SharePost"));
...
{
	UE_LOG(LogB3atZOnline, Verbose, TEXT("FTestSharingInterface::OnPostShared[PlayerIdx:%i - Successful:%i]"), LocalPlayer, bWasSuccessful);
...
{
	UE_LOG(LogB3atZOnline, Verbose, TEXT("FTestSharingInterface::RequestPermissionsToReadNewsFeed"));
	...
	// We need to be permitted to post on the users behalf to share this update
	EB3atZOnlineSharingReadCategory::Type ReadPermissions = EB3atZOnlineSharingReadCategory::Posts;
...
{
	UE_LOG(LogB3atZOnline, Display, TEXT("FTestSharingInterface::OnReadFeedPermissionsUpdated() - %d"), bWasSuccessful);
...
{
	UE_LOG(LogB3atZOnline, Verbose, TEXT("FTestSharingInterface::ReadNewsFeed"));
...
{
	UE_LOG(LogB3atZOnline, Display, TEXT("FTestSharingInterface::OnNewsFeedRead[PlayerIdx:%i - Successful:%i]"), LocalPlayer, bWasSuccessful);
	...
	if (bWasSuccessful)
	{
		...
		UE_LOG(LogB3atZOnline, Display, TEXT("FTestSharingInterface first read update: %s"), *FirstReadStatusUpdate.Message);
		...
		UE_LOG(LogB3atZOnline, Display, TEXT("FTestSharingInterface number of read updates: %d"), AllReadStatusUpdates.Num());

		for (int Idx = 0; Idx < AllReadStatusUpdates.Num(); ++Idx)
		{
			const FOnlineStatusUpdate& StatusUpdate = AllReadStatusUpdates[Idx];
			UE_LOG(LogB3atZOnline, Display, TEXT("FTestSharingInterface status update [%d]: %s"), Idx, *StatusUpdate.Message);
		}
	}
	...
	if( ++ResponsesReceived == MAX_LOCAL_PLAYERS )
	{
		UE_LOG(LogB3atZOnline, Display, TEXT("FTestSharingInterface TESTS COMPLETED"), LocalPlayer, bWasSuccessful);
		delete this;
	}
}
