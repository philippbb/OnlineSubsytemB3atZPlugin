...
{
	UE_LOG(LogB3atZOnline, Display, TEXT("FTestMessageInterface::FTestMessageInterface"));
...
{
	UE_LOG(LogB3atZOnline, Display, TEXT("FTestMessageInterface::Test"));
...
void FTestMessageInterface::OnReadMessageComplete(int32 LocalPlayer, bool bWasSuccessful, const FUniqueMessageId& MessageId, const FString& ErrorStr)
{
	...
	if (bWasSuccessful)
	{
...

		{
			...
			{
				UE_LOG(LogB3atZOnline, Log, TEXT("Attribute BLOBValue is missing from the received message payload"));
			}
		}
	}
