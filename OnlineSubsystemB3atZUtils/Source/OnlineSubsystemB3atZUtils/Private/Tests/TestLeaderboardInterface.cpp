...
public:
	TestLeaderboardWrite()
	{
		...
		SortMethod = EB3atZLeaderboardSort::Descending;
...
{
	UE_LOG(LogB3atZOnline, Verbose, TEXT("OnLeaderboardFlushComplete Session: %s bWasSuccessful: %d"), *SessionName.ToString(), bWasSuccessful);
...
{
	UE_LOG(LogB3atZOnline, Verbose, TEXT("OnLeaderboardReadComplete bWasSuccessful: %d"), bWasSuccessful);
	bOverallSuccess = bOverallSuccess && bWasSuccessful;

	for (int32 RowIdx = 0; RowIdx < ReadObject->Rows.Num(); ++RowIdx)
	{
		const FOnlineStatsRow& StatsRow = ReadObject->Rows[RowIdx];
		UE_LOG(LogB3atZOnline, Log, TEXT("Leaderboard stats for: Nickname = %s, Rank = %d"), *StatsRow.NickName, StatsRow.Rank);

		for (FStatsColumnArray::TConstIterator It(StatsRow.Columns); It; ++It)
		{
			UE_LOG(LogB3atZOnline, Log, TEXT("  %s = %s"), *It.Key().ToString(), *It.Value().ToString());
		}
	}
