...
FTestUserInterface::FTestUserInterface(const FString& InSubsystem)
	...
	, bQueryUserInfo(true)
{
	UE_LOG(LogB3atZOnline, Display, TEXT("FTestUserInterface::FTestUserInterface"));
...
{
	UE_LOG(LogB3atZOnline, Display, TEXT("FTestUserInterface::Test"));
...
void FTestUserInterface::OnQueryUserInfoComplete(int32 LocalPlayer, bool bWasSuccessful, const TArray< TSharedRef<const FUniqueNetId> >& UserIds, const FString& ErrorStr)
{
	...
	if (bWasSuccessful)
	{
		...
		{
			TSharedPtr<FB3atZOnlineUser> User = OnlineSub->GetUserInterface()->GetUserInfo(LocalPlayer, *UserIds[UserIdx]);
			if (User.IsValid())
			{
				UE_LOG(LogB3atZOnline, Log,
	...
	{
		UE_LOG(LogB3atZOnline, Error, TEXT("GetUserInterface() failure. Error = %s"), *ErrorStr);
	}
