...
/*=============================================================================
	IpNetDriver: Unreal IP network driver.
Notes:
...
UIpNetDriverB3atZ::FOnNetworkProcessingCausingSlowFrame UIpNetDriverB3atZ::OnNetworkProcessingCausingSlowFrame;
...
UIpNetDriverB3atZ::UIpNetDriverB3atZ(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

bool UIpNetDriverB3atZ::IsAvailable() const
{
	// IP driver always valid for now
	return true;
}

ISocketSubsystem* UIpNetDriverB3atZ::GetSocketSubsystem()
{
	return ISocketSubsystem::Get();
}

FSocket * UIpNetDriverB3atZ::CreateSocket()
{
	UE_LOG(LogNet, Warning, TEXT("IpNetDriver CreateSocket"));
	...
	{
		UE_LOG(LogNet, Warning, TEXT("UIpNetDriverB3atZ::CreateSocket: Unable to find socket subsystem"));
		return NULL;
	}
...
int UIpNetDriverB3atZ::GetClientPort()
{
	UE_LOG(LogNet, Warning, TEXT("IpNetDriver GetClientPort()"));
	
	return 0;
}

bool UIpNetDriverB3atZ::InitBase( bool bInitAsClient, FNetworkNotify* InNotify, const FURL& URL, bool bReuseAddressAndPort, FString& Error )
{
	UE_LOG(LogNet, Warning, TEXT("IpNetDriver InitBase"));

	if (!Super::InitBase(bInitAsClient, InNotify, URL, bReuseAddressAndPort, Error))
	{
		UE_LOG(LogNet, Warning, TEXT("IpNetDriver InitBase super couldnt be initiated"));
	...
	UE_LOG(LogInit, Warning, TEXT("%s: Socket queue %i / %i"), SocketSubsystem->GetSocketAPIName(), RecvSize, SendSize );
...
bool UIpNetDriverB3atZ::InitConnect( FNetworkNotify* InNotify, const FURL& ConnectURL, FString& Error )
{
	UE_LOG(LogNet, Warning, TEXT("IpNetDriver InitConnect with Port %u"), ConnectURL.Port);
	...
	UE_LOG(LogNet, Log, TEXT("Game client on ip %s port %i, rate %i"), *ConnectURL.Host ,ConnectURL.Port, ServerConnection->CurrentNetSpeed );
...
bool UIpNetDriverB3atZ::InitListen( FNetworkNotify* InNotify, FURL& LocalURL, bool bReuseAddressAndPort, FString& Error )
{
	UE_LOG(LogNet, Warning, TEXT("IPNetDriver InitListen"));
	...
	UE_LOG(LogNet, Log, TEXT("%s IpNetDriverB3atZ listening on ip %s port %i"), *GetDescription(), *LocalURL.Host, LocalURL.Port );

	return true;
}

void UIpNetDriverB3atZ::TickDispatch( float DeltaTime )
{
	...
	for( ; Socket != NULL; )
	{
		...
		else
		{
			...
			else
			{
				...
				{
					UIpConnectionB3atZ* Connection = nullptr;
		...
		UIpConnectionB3atZ* Connection = nullptr;
		UIpConnectionB3atZ* MyServerConnection = GetServerConnection();
		...
		{
			UIpConnectionB3atZ* TestConnection = (UIpConnectionB3atZ*)ClientConnections[i]; 
			...
			if( Connection )
			{
				if( Connection != GetServerConnection() )
				{
					...
					if ((Connection->State != USOCK_Open) || (!AllowPlayerPortUnreach))
					{
						if (LogPortUnreach)
						{
							UE_LOG(LogNet, Warning, TEXT("Received ICMP port unreachable from client %s.  Disconnecting."),
								*FromAddr->ToString(true));
						}
						Connection->CleanUp();
					}
			...
			if( !Connection )
			{
				...
				if (bAcceptingConnection)
				{
					...
					if (bPassedChallenge)
					{
						...
						Connection = NewObject<UIpConnectionB3atZ>(GetTransientPackage(), NetConnectionClass);
						check(Connection);
	...
	{
		UE_LOG( LogNet, Warning, TEXT( "UIpNetDriverB3atZ::TickDispatch: Took too long to receive packets. Time: %2.2f %s" ), DeltaReceiveTime, *GetName() );
	}
}

void UIpNetDriverB3atZ::LowLevelSend(FString Address, void* Data, int32 CountBits)
{
	UE_LOG(LogNet, Warning, TEXT("IpNetDriver (beatz) lowlevelsend to addr %s"), *Address);
	...
	{
		UE_LOG(LogNet, Warning, TEXT("UIpNetDriverB3atZ::LowLevelSend: Invalid send address '%s'"), *Address);
	}
}

void UIpNetDriverB3atZ::ProcessRemoteFunction(class AActor* Actor, UFunction* Function, void* Parameters, FOutParmRec* OutParms, FFrame* Stack, class UObject* SubObject )
{
	...
	{
		UE_LOG(LogNet, Warning, TEXT("UIpNetDriverB3atZ::ProcesRemoteFunction: No owning connection for actor %s. Function %s will not be processed."), *Actor->GetName(), *Function->GetName());
	}
}

FString UIpNetDriverB3atZ::LowLevelGetNetworkNumber()
{
	UE_LOG(LogNet, Warning, TEXT("IpNetDriver  LowLevelGetNetworkNumber is %s"), *LocalAddr->ToString(true));

	return LocalAddr->ToString(true);
}

void UIpNetDriverB3atZ::LowLevelDestroy()
{
...
bool UIpNetDriverB3atZ::HandleSocketsCommand( const TCHAR* Cmd, FOutputDevice& Ar, UWorld* InWorld )
{
	Ar.Logf(TEXT(""));
	if (Socket != NULL)
	{
...
bool UIpNetDriverB3atZ::Exec( UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar )
{
...
UIpConnectionB3atZ* UIpNetDriverB3atZ::GetServerConnection() 
{
	return (UIpConnectionB3atZ*)ServerConnection;
}
