...
AB3atZOnlineBeaconClient::AB3atZOnlineBeaconClient(const FObjectInitializer& ObjectInitializer) :
	Super(ObjectInitializer),
	BeaconOwner(nullptr),
	BeaconConnection(nullptr),
	ConnectionState(EDirectBeaconConnectionState::Invalid)
{
	NetDriverName = FName(TEXT("BeaconDriverClient"));
	bOnlyRelevantToOwner = true;
}

FString AB3atZOnlineBeaconClient::GetBeaconType() const
{
	return GetClass()->GetName();
}

AB3atZOnlineBeaconHostObject* AB3atZOnlineBeaconClient::GetBeaconOwner() const
{
	return BeaconOwner;
}

void AB3atZOnlineBeaconClient::SetBeaconOwner(AB3atZOnlineBeaconHostObject* InBeaconOwner)
{
	BeaconOwner = InBeaconOwner;
}

const AActor* AB3atZOnlineBeaconClient::GetNetOwner() const
{
	return BeaconOwner;
}

class UNetConnection* AB3atZOnlineBeaconClient::GetNetConnection() const
{
	return BeaconConnection;
}

bool AB3atZOnlineBeaconClient::DestroyNetworkActorHandled()
{
	if (BeaconConnection)
	{
...
const FUniqueNetIdRepl& AB3atZOnlineBeaconClient::GetUniqueId() const
{
	if (BeaconConnection)
	{
...
EDirectBeaconConnectionState AB3atZOnlineBeaconClient::GetConnectionState() const
{
	return ConnectionState;
}

void AB3atZOnlineBeaconClient::SetConnectionState(EDirectBeaconConnectionState NewConnectionState)
{
	ConnectionState = NewConnectionState;
}

bool AB3atZOnlineBeaconClient::InitClient(FURL& URL)
{
	bool bSuccess = false;
	...
	if(URL.Valid)
	{
		if (InitBase() && NetDriver)
		{
			...
			if (NetDriver->InitConnect(this, URL, Error))
			{
				...
				SetConnectionState(EDirectBeaconConnectionState::Pending);
			...
			else
			{
				...
				UE_LOG(LogBeacon, Log, TEXT("AB3atZOnlineBeaconClient::InitClient failed"));
				SetConnectionState(EDirectBeaconConnectionState::Invalid);
				OnFailure();
			}
		}
	}
...
void AB3atZOnlineBeaconClient::OnFailure()
{
...
void AB3atZOnlineBeaconClient::ClientOnConnected_Implementation()
{
	SetConnectionState(EDirectBeaconConnectionState::Open);
...
bool AB3atZOnlineBeaconClient::UseShortConnectTimeout() const
{
	return ConnectionState == EDirectBeaconConnectionState::Open;
}

void AB3atZOnlineBeaconClient::DestroyBeacon()
{
	SetConnectionState(EDirectBeaconConnectionState::Closed);
...
void AB3atZOnlineBeaconClient::OnNetCleanup(UNetConnection* Connection)
{
	ensure(Connection == BeaconConnection);
	SetConnectionState(EDirectBeaconConnectionState::Closed);

	AB3atZOnlineBeaconHostObject* BeaconHostObject = GetBeaconOwner();
	if (BeaconHostObject)
	{
...
void AB3atZOnlineBeaconClient::NotifyControlMessage(UNetConnection* Connection, uint8 MessageType, class FInBunch& Bunch)
{
		...
		case NMT_BeaconAssignGUID:
			{
				...
				if (NetGUID.IsValid())
				{
					...
					// Fail safe for connection to server but no client connection RPC
					FTimerDelegate TimerDelegate = FTimerDelegate::CreateUObject(this, &AB3atZOnlineBeaconClient::OnFailure);
