...
#include "B3atZPartyBeaconClient.h"

ADirectPartyBeaconHost::ADirectPartyBeaconHost(const FObjectInitializer& ObjectInitializer) :
	Super(ObjectInitializer),
	State(NULL),
	bLogoutOnSessionTimeout(true)
{
	ClientBeaconActorClass = AB3atZPartyBeaconClient::StaticClass();
...
void ADirectPartyBeaconHost::PostInitProperties()
{
...
bool ADirectPartyBeaconHost::InitHostBeacon(int32 InTeamCount, int32 InTeamSize, int32 InMaxReservations, FName InSessionName, int32 InForceTeamNum)
{
	...
	{
		State = NewObject<UB3atZPartyBeaconState>(GetTransientPackage(), GetPartyBeaconHostClass());
...
bool ADirectPartyBeaconHost::InitFromBeaconState(UB3atZPartyBeaconState* PrevState)
{
	if (!State && PrevState)
	{
...
bool ADirectPartyBeaconHost::ReconfigureTeamAndPlayerCount(int32 InNumTeams, int32 InNumPlayersPerTeam, int32 InNumReservations)
{
	bool bSuccess = false;
...
void ADirectPartyBeaconHost::SetTeamAssignmentMethod(FName NewAssignmentMethod)
{
	if (State)
	{
...
void ADirectPartyBeaconHost::Tick(float DeltaTime)
{
	if (State)
	{
		...
		if (SessionInt.IsValid())
		{
			...
			TArray<FB3atZPartyReservation>& Reservations = State->GetReservations();
			...
			if (Session)
			{
				...
				for (int32 ResIdx = 0; ResIdx < Reservations.Num(); ResIdx++)
				{
					...
					{
						AB3atZPartyBeaconClient* Client = Cast<AB3atZPartyBeaconClient>(ClientActors[ClientIdx]);
						if (Client)
						{
							const FB3atZPartyReservation& ClientRes = Client->GetPendingReservation();
					...
					if (bIsConnectedReservation)
					{
						...
						{
							FB3atZPlayerReservation& PlayerEntry = PartyRes.PartyMembers[PlayerIdx];
					...
					else
					{
						for (int32 PlayerIdx = 0; PlayerIdx < PartyRes.PartyMembers.Num(); PlayerIdx++)
						{
							...
							if (SessionInt->IsPlayerInSession(SessionName, *PlayerEntry.UniqueId) ||
								...
								bIsSessionOwner)
							{
								FB3atZUniqueNetIdMatcher PlayerMatch(*PlayerEntry.UniqueId);
				...
				if (bLogoutOnSessionTimeout)
				{
					...
					for (int32 LogoutIdx = 0; LogoutIdx < PlayersToLogout.Num(); LogoutIdx++)
					{
						...
						for (int32 ResIdx = 0; ResIdx < Reservations.Num(); ResIdx++)
						{
							const FB3atZPartyReservation& PartyRes = Reservations[ResIdx];
							for (int32 PlayerIdx = 0; PlayerIdx < PartyRes.PartyMembers.Num(); PlayerIdx++)
							{
								const FB3atZPlayerReservation& PlayerEntry = PartyRes.PartyMembers[PlayerIdx];
...
int32 ADirectPartyBeaconHost::GetNumPlayersOnTeam(int32 TeamIdx) const
{
	int32 Result = 0;
	if (GetOwner() != NULL && State != NULL)
	{
...
int32 ADirectPartyBeaconHost::GetTeamForCurrentPlayer(const FUniqueNetId& PlayerId) const
{
...
int32 ADirectPartyBeaconHost::GetPlayersOnTeam(int32 TeamIndex, TArray<FUniqueNetIdRepl>& TeamMembers) const
{
	if (State)
	{
...
void ADirectPartyBeaconHost::SendReservationUpdates()
{
	if (State && ClientActors.Num() > 0)
	{
		...
		if (NumRemaining < MaxReservations)
		{
			if (NumRemaining > 0)
			{
				...
				{
					AB3atZPartyBeaconClient* PartyBeaconClient = Cast<AB3atZPartyBeaconClient>(ClientActor);
...
void ADirectPartyBeaconHost::NewPlayerAdded(const FB3atZPlayerReservation& NewPlayer)
{
...
void ADirectPartyBeaconHost::NotifyReservationEventNextFrame(FOnReservationUpdate& ReservationEvent)
{
	UWorld* World = GetWorld();
	check(World);
...
void ADirectPartyBeaconHost::HandlePlayerLogout(const FUniqueNetIdRepl& PlayerId)
{
	if (PlayerId.IsValid())
	{
...
bool ADirectPartyBeaconHost::SwapTeams(const FUniqueNetIdRepl& PartyLeader, const FUniqueNetIdRepl& OtherPartyLeader)
{
	bool bSuccess = false;
	if (State)
	{
...
bool ADirectPartyBeaconHost::ChangeTeam(const FUniqueNetIdRepl& PartyLeader, int32 NewTeamNum)
{
	bool bSuccess = false;
	if (State)
	{
...
bool ADirectPartyBeaconHost::PlayerHasReservation(const FUniqueNetId& PlayerId) const
{
...
bool ADirectPartyBeaconHost::GetPlayerValidation(const FUniqueNetId& PlayerId, FString& OutValidation) const
{
	OutValidation.Empty();
...
bool ADirectPartyBeaconHost::GetPartyLeader(const FUniqueNetIdRepl& InPartyMemberId, FUniqueNetIdRepl& OutPartyLeaderId) const
{
...
EB3atZPartyReservationResult::Type ADirectPartyBeaconHost::AddPartyReservation(const FB3atZPartyReservation& ReservationRequest)
{
	EB3atZPartyReservationResult::Type Result = EB3atZPartyReservationResult::GeneralError;

	if (!State || GetBeaconState() == EBeaconState::DenyRequests)
	{
		return EB3atZPartyReservationResult::ReservationDenied;
	}
	...
	if (ReservationRequest.IsValid())
	{
		...
		{
			TArray<FB3atZPartyReservation>& Reservations = State->GetReservations();
			FB3atZPartyReservation& ExistingReservation = Reservations[ExistingReservationIdx];
			if (ReservationRequest.PartyMembers.Num() == ExistingReservation.PartyMembers.Num())
			{
				...
				for (const FB3atZPlayerReservation& NewPlayerRes : ReservationRequest.PartyMembers)
				{
					FB3atZPlayerReservation* PlayerRes = ExistingReservation.PartyMembers.FindByPredicate(
						[NewPlayerRes](const FB3atZPlayerReservation& ExistingPlayerRes)
					{
				...
				if (NumMatchingReservations == ExistingReservation.PartyMembers.Num())
				{
					...
					Result = EB3atZPartyReservationResult::ReservationDuplicate;
				}
				else
				{
					// Existing reservation doesn't match incoming duplicate reservation
					Result = EB3atZPartyReservationResult::IncorrectPlayerCount;
				}
			}
			else
			{
			...
			if (State->DoesReservationFit(ReservationRequest))
				{
					...
					if (State->AreTeamsAvailable(ReservationRequest))
					{
						if (State->AddReservation(ReservationRequest))
						{
							...
							for (const FB3atZPlayerReservation& PartyMember : ReservationRequest.PartyMembers)
							{
							...
							Result = EB3atZPartyReservationResult::ReservationAccepted;
						}
						else
						{
					...
					else
					{
						...
						Result = EB3atZPartyReservationResult::PartyLimitReached;
					}
				}
				else
				{
					// Validate players failed above
					Result = EB3atZPartyReservationResult::ReservationDenied_Banned;
				}
			}
			else
			{
	...
	{
		// Invalid reservation
		Result = EB3atZPartyReservationResult::ReservationInvalid;
	}

	return Result;
}

EB3atZPartyReservationResult::Type ADirectPartyBeaconHost::UpdatePartyReservation(const FB3atZPartyReservation& ReservationUpdateRequest)
{
	EB3atZPartyReservationResult::Type Result = EB3atZPartyReservationResult::GeneralError;
	...
	if (ReservationUpdateRequest.IsValid())
	{
		if (!State->IsBeaconFull())
		{
			...
			if (ExistingReservationIdx != INDEX_NONE)
			{
				...
				TArray<FB3atZPlayerReservation> NewPlayers;
				for (int32 PlayerIdx = 0; PlayerIdx < ReservationUpdateRequest.PartyMembers.Num(); PlayerIdx++)
				{
					const FB3atZPlayerReservation& NewPlayerRes = ReservationUpdateRequest.PartyMembers[PlayerIdx];
				...
				if (NewPlayers.Num() <= NumAvailableSlotsOnTeam)
				{
					if (NewPlayers.Num() > 0)
					{
						...
						{
							const FB3atZPlayerReservation& PlayerRes = NewPlayers[PlayerIdx];
			...
			else
			{
				...
				Result = EB3atZPartyReservationResult::ReservationNotFound;
			}
		}
		else
		{
...
EB3atZPartyReservationResult::Type ADirectPartyBeaconHost::RemovePartyReservation(const FUniqueNetIdRepl& PartyLeader)
{
	if (State && State->RemoveReservation(PartyLeader))
	{
		...
		SendReservationUpdates();
		NotifyReservationEventNextFrame(ReservationChanged);
		return EB3atZPartyReservationResult::ReservationRequestCanceled;
	}
	else
	{
		...
		return EB3atZPartyReservationResult::ReservationNotFound;
	}
}

void ADirectPartyBeaconHost::RegisterAuthTicket(const FUniqueNetIdRepl& InPartyMemberId, const FString& InAuthTicket)
{
	if (State)
	{
...
void ADirectPartyBeaconHost::UpdatePartyLeader(const FUniqueNetIdRepl& InPartyMemberId, const FUniqueNetIdRepl& NewPartyLeaderId)
{
	if (State)
	{
...
bool ADirectPartyBeaconHost::DoesSessionMatch(const FString& SessionId) const
{
	if (State)
	{
		UWorld* World = GetWorld();
...
void ADirectPartyBeaconHost::ProcessReservationRequest(AB3atZPartyBeaconClient* Client, const FString& SessionId, const FB3atZPartyReservation& ReservationRequest)
{
	...
	if (Client)
	{
		EB3atZPartyReservationResult::Type Result = EB3atZPartyReservationResult::BadSessionId;
		if (DoesSessionMatch(SessionId))
		{
			Result = AddPartyReservation(ReservationRequest);
		}

		UE_LOG(LogBeacon, Verbose, TEXT("ProcessReservationRequest result: %s"), EB3atZPartyReservationResult::ToString(Result));
...
void ADirectPartyBeaconHost::ProcessReservationUpdateRequest(AB3atZPartyBeaconClient* Client, const FString& SessionId, const FB3atZPartyReservation& ReservationUpdateRequest)
{
	...
	if (Client)
	{
		...
		UE_LOG(LogBeacon, Verbose, TEXT("ProcessReservationUpdateRequest result: %s"), EB3atZPartyReservationResult::ToString(Result));
		if (UE_LOG_ACTIVE(LogBeacon, Verbose) &&
			(Result != EB3atZPartyReservationResult::ReservationAccepted))
		{
			DumpReservations();
...
void ADirectPartyBeaconHost::ProcessCancelReservationRequest(AB3atZPartyBeaconClient* Client, const FUniqueNetIdRepl& PartyLeader)
{
	...
	if (Client)
	{
		EB3atZPartyReservationResult::Type Result = RemovePartyReservation(PartyLeader);
		UE_LOG(LogBeacon, Verbose, TEXT("ProcessCancelReservationRequest result: %s"), EB3atZPartyReservationResult::ToString(Result));
		if (UE_LOG_ACTIVE(LogBeacon, Verbose) &&
			(Result != EB3atZPartyReservationResult::ReservationRequestCanceled))
		{
			DumpReservations();
		}
...
void ADirectPartyBeaconHost::DumpReservations() const
{
