	...
	if (OnlineSubsystem)
	{
			...
			VoiceEngine.IsValid())
		{
			...
			if (Talker == NULL)
			{
				...
				// Copy the UniqueId
				const FB3atZUniqueNetIdString& UniqueIdStr = (const FB3atZUniqueNetIdString&)UniqueId;
				Talker->TalkerId = MakeShareable(new FB3atZUniqueNetIdString(UniqueIdStr));
...
bool FOnlineB3atZVoiceImpl::MuteRemoteTalker(uint8 LocalUserNum, const FUniqueNetId& PlayerId, bool bIsSystemWide)
{
	...
	if (LocalUserNum >= 0 && LocalUserNum < MaxLocalTalkers )
	{
		if (bIsSystemWide)
		{
			...
			SystemMuteList.AddUnique((const FB3atZUniqueNetIdString&)PlayerId);
		...
		else
		{
					...
					VoiceEngine.IsValid())
			{
				...
				if (Talker != NULL)
				{
					MuteList.AddUnique((const FB3atZUniqueNetIdString&)PlayerId);
					Return = S_OK;
...
bool FOnlineB3atZVoiceImpl::UnmuteRemoteTalker(uint8 LocalUserNum, const FUniqueNetId& PlayerId, bool bIsSystemWide)
{
	...
	if (LocalUserNum >= 0 && LocalUserNum < MaxLocalTalkers )
	{
		if (bIsSystemWide)
		{
			...
			SystemMuteList.RemoveSingleSwap((const FB3atZUniqueNetIdString&)PlayerId);
			// Should update MuteList after going up to the server and coming back down
			ProcessMuteChangeNotification();
		}
		else
				...
				VoiceEngine.IsValid())
			{
				...
				if (Talker != NULL)
				{
					...
					if (!bIsSystemMuted)
					{
						...
						MuteList.RemoveSingleSwap((const FB3atZUniqueNetIdString&)PlayerId);
...
void FOnlineB3atZVoiceImpl::UpdateMuteListForLocalTalker(int32 TalkerIndex, APlayerController* PlayerController)
{
	...
	for (int32 RemoteIndex = 0; RemoteIndex < RemoteTalkers.Num(); RemoteIndex++)
	{
		...
		if (SystemMuteList.Find((FB3atZUniqueNetIdString&)*Talker.TalkerId) == INDEX_NONE)
		{
...
TSharedPtr<FVoicePacket> FOnlineB3atZVoiceImpl::SerializeRemotePacket(FArchive& Ar)
{
	...
	if (Ar.IsError() == false && NewPacket->GetBufferSize() > 0)
	{
		if (!OnlineSubsystem->IsDedicated())
		{
			FB3atZUniqueNetIdMatcher PlayerMatch(*NewPacket->GetSender());
